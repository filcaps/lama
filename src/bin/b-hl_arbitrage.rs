...
use binance_api::client::{Binance, BinanceEnv};
use hyperliquid_api::client::{Hyperliquid, HyperliquidEnv};
use binance_arbitrage::broker::gatekeeper::kucoin::task_gatekeep_chances;
use hyperliquid_arbitrage::broker::gatekeeper::kucoin::task_gatekeep_chances;
use binance_arbitrage::broker::order::kucoin::task_place_order;
use hyperliquid_arbitrage::broker::order::kucoin::task_place_order;
use binance_arbitrage::broker::orderbook::internal::task_sync_orderbook;
use hyperliquid_arbitrage::broker::orderbook::internal::task_sync_orderbook;
use binance_arbitrage::broker::orderbook::binance::{
    task_get_initial_orderbooks, task_pub_orderbook_event,
};
use hyperliquid_arbitrage::broker::orderbook::hyperliquid::{
    task_get_initial_orderbooks, task_pub_orderbook_event,
};
use binance_arbitrage::broker::symbol::filter::{symbol_with_quotes, vector_to_hash};
use hyperliquid_arbitrage::broker::symbol::filter::{symbol_with_quotes, vector_to_hash};
use binance_arbitrage::broker::symbol::binance::{format_subscription_list, get_symbols};
use hyperliquid_arbitrage::broker::symbol::hyperliquid::{format_subscription_list, get_symbols};
use binance_arbitrage::broker::trade::binance::task_pub_trade_event;
use hyperliquid_arbitrage::broker::trade::hyperliquid::task_pub_trade_event;
use binance_arbitrage::event::{
    chance::ChanceEvent, order::OrderEvent, orderbook::OrderbookEvent, trade::TradeEvent,
};
use hyperliquid_arbitrage::event::{
    chance::ChanceEvent, order::OrderEvent, orderbook::OrderbookEvent, trade::TradeEvent,
};
use binance_arbitrage::model::orderbook::FullOrderbook;
use hyperliquid_arbitrage::model::orderbook::FullOrderbook;
use binance_arbitrage::monitor::counter::Counter;
use hyperliquid_arbitrage::monitor::counter::Counter;
use binance_arbitrage::monitor::task::{task_log_mps, task_monitor_channel_mps};
use hyperliquid_arbitrage::monitor::task::{task_log_mps, task_monitor_channel_mps};
use binance_arbitrage::strategy::all_taker_rdnt_usdt::task_pub_chance_all_taker_rdnt_usdt;
use hyperliquid_arbitrage::strategy::all_taker_rdnt::task_pub_chance_all_taker_rdnt;
use binance_arbitrage::system_event::task_signal_handle;
use hyperliquid_arbitrage::system_event::task_signal_handle;
use std::sync::Arc;
...
async fn main() -> Result<()> {
    ...
    let _worker_guard = b-hl_arbitrage::logger::setup_logs(&config.log)?;
    tracing::info!("Log setup");
    tokio::select! {
...
async fn core(config: b-hl_arbitrage::config::Config) -> Result<()> {
    // config parameters
    let budget = config.behaviour.usd_cyclic_arbitrage;
    let monitor_interval = config.behaviour.monitor_interval_sec;

    // API endpoints
    let api = Binance::new(BinanceEnv::Live, Some(config.binance_credentials()))
        .map_err(|e| eyre::eyre!(e))?;
    tracing::info!("Credentials setup");
   let api = Binance::new(BinanceEnv::Live, Some(config.hyperliquid_credentials()))
        .map_err(|e| eyre::eyre!(e))?;
    tracing::info!("Credentials setup");
    ...
    let symbol_infos = symbol_with_quotes(&symbol_list, "RDNT", "USDT");
